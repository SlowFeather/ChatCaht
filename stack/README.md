...
The `audio:` section in the ChatCaht config is the single source of truth for native audio settings. Managed commands generate `artifacts/run/audio-runtime.generated.json` atomically and pass that file to AudioRuntime; the initial WebSocket status is checked against the same settings before a voice session starts.
